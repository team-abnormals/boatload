...
public class BoatloadRecipeProvider extends RecipeProvider {
	...
	public BoatloadRecipeProvider(PackOutput generator) {
		super(generator);
	}
	...
	private static void furnaceBoat(Consumer<FinishedRecipe> consumer, BoatloadBoatItem furnaceBoat) {
		ShapelessRecipeBuilder.shapeless(RecipeCategory.TRANSPORTATION, furnaceBoat).requires(Blocks.FURNACE).requires(furnaceBoat.getType().boat().get()).group("furnace_boat").unlockedBy("has_boat", has(ItemTags.BOATS)).save(consumer);
	}

	private static void largeBoat(Consumer<FinishedRecipe> consumer, BoatloadBoatItem largeBoat) {
		ShapedRecipeBuilder.shaped(RecipeCategory.TRANSPORTATION, largeBoat).define('#', largeBoat.getType().planks().get()).define('B', largeBoat.getType().boat().get()).pattern("#B#").pattern("###").group("large_boat").unlockedBy("has_boat", has(ItemTags.BOATS)).save(consumer);
	}
}