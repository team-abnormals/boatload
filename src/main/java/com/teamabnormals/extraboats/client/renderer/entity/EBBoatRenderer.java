...
@OnlyIn(Dist.CLIENT)
public class EBBoatRenderer extends EntityRenderer<EBBoat> {
	private final Map<EBBoatType, Pair<ResourceLocation, BoatModel>> boatResources;

	public EBBoatRenderer(EntityRendererProvider.Context context) {
		super(context);
		this.shadowRadius = 0.8F;
		this.boatResources = Stream.of(EBBoatType.values()).collect(ImmutableMap.toImmutableMap((type) -> type, (boatType) -> Pair.of(new ResourceLocation(boatType.getModID(), "textures/entity/boat/" + boatType.getName() + ".png"), new BoatModel(context.bakeLayer(EBModelLayers.createBoatModelName(boatType))))));
	}

	public void render(EBBoat entityIn, float entityYaw, float partialTicks, PoseStack matrixStackIn, MultiBufferSource bufferIn, int packedLightIn) {
	...
	@Override
	public ResourceLocation getTextureLocation(EBBoat boat) {
		return getModelWithLocation(boat).getFirst();
	}

	public Pair<ResourceLocation, BoatModel> getModelWithLocation(EBBoat boat) {
